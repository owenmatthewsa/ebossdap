...
master_wave =  pd.read_csv(r"C:\Users\owenm\Documents\catalog_paper\eBOSSDAP\c3k\SpecAge13-0000Met2-5.csv").lam
for ssp in ssps:
    ...
    #sres[0] = sres[1]
    old_sres = sres_df['sres']
    r = sampling.Resample(y = np.array(old_sres), x = np.array(wave), newx = newwave)
    sres = interp1d(wave,old_sres,bounds_error=False,fill_value='extrapolate')(newwave)
    ...
    flux = data['flux'][1400:9500]
    r = sampling.Resample(y = np.array(flux), x = np.array(wave), newx = newwave)
    out_flux = r.outy
